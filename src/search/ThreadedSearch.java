...
public class ThreadedSearch<T> implements Runnable {
	...
	public boolean parSearch(int numThreads, T target, ArrayList<T> list) throws InterruptedException {
		/*
		 * First construct an instance of the `Answer` inner class. This will be
		 * how the threads you're about to create will "communicate". They will
		 * all have access to this one shared instance of `Answer`, where they
		 * can update the `answer` field inside that instance.
		 *
		 * Then construct `numThreads` instances of this class
		 * (`ThreadedSearch`) using the 5 argument constructor for the class.
		 * You'll hand each of them the same `target`, `list`, and `answer`.
		 * What will be different about each instance is their `begin` and `end`
		 * values, which you'll use to give each instance the "job" of searching
		 * a different segment of the list. If, for example, the list has length
		 * 100 and you have 4 threads, you would give the four threads the
		 * ranges [0, 25), [25, 50), [50, 75), and [75, 100) as their sections
		 * to search.
		 *
		...
		for (int i = 0; i < numThreads; i++) {
			threads[i] = new Thread
					(new ThreadedSearch(target, list, range * i, range * (i + 1), answer));
			threads[i].start();
		}
